...
import { QuestionCategory, ReadingQuestion } from '@/types';

export default function BookmarkedQuizScreen() {
  ...
  const bookmarkedQuestions = useMemo(() => {
    ...
    return getBookmarkedQuestions(questions, cat).sort(() => Math.random() - 0.5);
  }, [category]);
  ...
  const currentQuestion = bookmarkedQuestions[currentIndex];
  ...
  const handleNext = useCallback(() => {
    ...
    setIsAnswered(false);
  }, [currentIndex, bookmarkedQuestions.length]);

  if (bookmarkedQuestions.length === 0) {
    return (
      <View style={[styles.container, { paddingTop: insets.top }]}>
        <LinearGradient colors={[Colors.primary, Colors.primaryLight]} style={styles.emptyContainer}>
          <Bookmark size={48} color={Colors.accent} />
          <Text style={styles.emptyTitle}>Kayıtlı Soru Yok</Text>
          <Text style={styles.emptyText}>Soruları çözerken yer imi ekleyerek burada tekrar edebilirsin.</Text>
  ...
  if (isFinished) {
    const percentage = bookmarkedQuestions.length > 0 ? Math.round((score / bookmarkedQuestions.length) * 100) : 0;
    return (
      <View style={[styles.container, { paddingTop: insets.top }]}>
        <LinearGradient colors={[Colors.primary, Colors.primaryLight]} style={styles.emptyContainer}>
          ...
          <Text style={styles.emptyText}>
            {bookmarkedQuestions.length} kayıtlı sorudan {score} doğru (%{percentage})
          </Text>
          <TouchableOpacity style={styles.emptyButton} onPress={() => router.back()}>
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      <View style={styles.topBar}>
        ...
        <View style={styles.progressInfo}>
          ...
          <Text style={styles.progressText}>Kayıtlı {currentIndex + 1}/{bookmarkedQuestions.length}</Text>
        </View>
          ...
          <TouchableOpacity style={styles.nextButton} onPress={handleNext}>
            <LinearGradient colors={[Colors.accent, Colors.accentLight]} style={styles.nextGradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
              <Text style={styles.nextText}>
                {currentIndex >= bookmarkedQuestions.length - 1 ? 'Sonuçları Gör' : 'Sonraki'}
              </Text>
